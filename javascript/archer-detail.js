...
// - Simple tab switching and basic CRUD actions (edit, delete, back)
...
function deleteCurrentArcher() {
  if (!currentArcher) return;
  if (!confirm(`Delete ${currentArcher.first} ${currentArcher.last}?\nThis will remove the archer and their scores.`)) return;
  const data = loadData();
  if (!data) return;
  data.archers = (data.archers || []).filter(a => a.id !== currentArcher.id);
